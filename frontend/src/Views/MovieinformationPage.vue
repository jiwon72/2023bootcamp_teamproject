...
<div class="info-movie-item">
  ...
  <div class="info-movie-details">
    <div class="info-movie-detail-line">
      <ul><h3 class="info-movie-title" v-if="movie">{{ movie.title }}</h3></ul>
    </div>
    <table class="info-movie-details">
      <tr class="info-movie-detail-line">
        <td><h4>감독</h4></td>
        <td><span v-if="movie">{{ movie.director }}</span></td>
        <td><h4>언어</h4></td>
        <td><span v-if="movie">{{ movie.original_language }}</span></td>
        <td><h4>배급</h4></td>
        <td><span v-if="movie">{{ movie.distributor }}</span></td>
      </tr>

      <tr class="info-movie-detail-line">
        <td><h4>개봉 연도</h4></td>
        <td><span v-if="movie">{{ movie.release_date }}</span></td>
        <td><h4>장르</h4></td>
        <td><span v-if="movie">{{ getgenreName(movie.genres) }}</span></td>
        <td><h4>등급</h4></td>
        <td><span v-if="movie">{{ movie.director }}</span></td>
      </tr>
...
export default {
  ...
  try {
    const movieId = this.$store.state.selectedMovieId || this.$route.params.id; // Vuex 상태 또는 라우터 파라미터로부터 movieId 가져오기
